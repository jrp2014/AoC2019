...
parse :: String -> [Delta]
parse w = map parseSeg $ splitOn "," w
  where
    parseSeg ('L':len) = Delta (read len) (-1, 0)
    parseSeg ('R':len) = Delta (read len) (1, 0)
    parseSeg ('U':len) = Delta (read len) (0, -1)
    parseSeg ('D':len) = Delta (read len) (0, 1)

addDelta :: Coord -> Coord -> Coord
addDelta (x, y) (dx, dy) = (x + dx, y + dy)

expandPath :: [Delta] -> [Coord]
expandPath =
  scanl1 addDelta . concatMap (\(Delta len delta) -> replicate len delta)

-- Map of coords to steps to get there
steps :: [Delta] -> Map.Map Coord Int
steps deltas = Map.fromListWith min (zip (expandPath deltas) [1 ..])

intersections :: [[Delta]] -> Map.Map Coord Int
intersections = foldl1' (Map.intersectionWith (+)) . map steps

manhattan :: Coord -> Int
manhattan (x, y) = abs x + abs y

closestToOrigin :: Map.Map Coord Int -> Int
closestToOrigin = minimum . map manhattan . Map.keys
...
main :: IO ()
main = do
  let pwire1 = parse wire1
  let pwire2 = parse wire2
  let wireIntersections = intersections [pwire1, pwire2]
  putStr "Part 1: "
  print $ closestToOrigin wireIntersections
  putStr "Part 2: "
  print $ minimum wireIntersections
